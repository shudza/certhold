# Certhold: SSH Manager — Design Plan
...
**One trust root, one actor type.** The manager runs an SSH Certificate Authority. Every device on the network — including the manager itself — is a "peer." Each peer holds a CA-signed certificate listing the groups it belongs to. Peers grant access to other peers based on group principal matching, enforced by `sshd` natively.

**No expiries, no agents.** Certificates are issued without expiration. Peers run no software beyond OpenSSH. State changes are pushed by the manager over SSH using its own peer cert.

**The manager is a peer.** It self-enrolls at init, gets a cert with a privileged `manager` principal that every peer accepts as root. All manager → peer operations use this cert. No bespoke authentication anywhere in the system.

## Architecture

Two components:

**Manager** — a single Go binary running on one machine in the home network. Exposes:
...
/etc/ssh/sshd_config.d/manager.conf      # sshd directives
/etc/ssh/auth_principals/root            # one group name per line
/etc/ssh/ca_known_hosts                  # @cert-authority entry for outbound SSH
/etc/ssh/ssh_config.d/manager.conf       # client-side cert config
/root/.ssh/authorized_keys               # contains the manager principal indirectly via auth_principals
```

`sshd_config.d/manager.conf`:
```
...
Every peer's `auth_principals/root` implicitly includes `manager` (added at onboarding, before the user-specified groups). This is what gives the manager standing access.

## Onboarding flow

**Admin runs on manager:**
```
$ ssh-manager enroll new-vm --groups infra,databases
echo "BASE64..." | bash
```
...
1. Curls `https://manager.home.lan/enroll/<token>` and pipes the response (a base64-encoded tarball) into `tar -xzC /`
2. Reloads sshd
...
```
ssh-manager init                          # generate CA, self-enroll the manager as a peer
ssh-manager enroll <name> --groups a,b    # mint a token, print onboarding one-liner
ssh-manager list [--peers|--groups]       # show state
ssh-manager update <name> --groups a,b,d  # reissue cert with new principals, push to peer, reload sshd
ssh-manager group allow <group> --on <peer>      # add group to peer's auth_principals/root, push
ssh-manager group disallow <group> --on <peer>   # remove from auth_principals/root, push
ssh-manager revoke <name>                 # add cert serial to KRL, regenerate KRL, push to all peers
ssh-manager rekey                         # rotate CA: new keypair, new certs for every peer, push atomically
```

Every operation except `init` and `enroll` involves the manager SSHing into one or more peers using its own cert. The manager has no other authentication mechanism to peers.

## Push operations

When the manager modifies a peer (cert reissue, principals update, KRL push), the flow is:

1. SSH into the peer using the manager's cert
...
2. For each peer (including the manager itself, last):
...
If rekey fails partway, the operation is resumable: peers already updated have the new CA in place, and the manager can be reissued certs against the new CA to continue. For v1, manual recovery on failure is acceptable.
...
1. Reduce the blast radius of the manager principal.
Right now manager = root on every peer, forever. A few cheap changes:

Drop the universal principal, push per-operation certs. Instead of the manager holding one long-lived cert with the manager principal, it signs itself a short-TTL cert (say 5 minutes) right before each push operation, scoped to just the peers it needs to touch. The CA key is only "hot" during operations. Between operations, no cert in existence grants manager-level access.
Force-command on the manager principal. auth_principals/root could map manager to a restricted command set via authorized_keys command= or sshd Match Principal blocks — essentially "the manager can only run these specific atomic-file-swap and reload-sshd operations, not arbitrary shell." Limits what a compromised manager can do per-host even if it has the cert. Fiddly to get right but matches your "push model" cleanly since the operations are a fixed set.

2. Get the CA key off the manager.
...
TPM-backed CA key. Most modern hardware has a TPM 2.0. You can generate the CA key inside the TPM and sign through it — the key never exists as a file. An attacker with root on the manager can still request signatures while they're on the box, but can't exfiltrate the key to use later or elsewhere. ssh-tpm-agent or direct PKCS#11 via tpm2-pkcs11 both work with ssh-keygen. This is probably the single highest-leverage mitigation.
YubiKey / hardware token. Same idea, removable. CA key on a YubiKey, plugged in only when you're actively administering. Great for homelab — matches the "I'm at my desk doing admin" workflow. Touch-to-sign means even with root on the manager, an attacker can't sign new certs without physical presence.
...
That last one is the binding constraint. Your sshd_config.d/manager.conf only gets read if the main sshd_config has Include /etc/ssh/sshd_config.d/*.conf — and that directive doesn't parse on anything older than 8.2.
...
- **DNS / hostname resolution.** Peers reach each other by IP for v1. A DNS server on the manager is a clear v2 addition.
- **Cert expiries.** Certs are issued without `-V`. The architecture supports adding TTLs later without changes — just start issuing with `-V +24h` and add a cert-refresh push to the manager's CLI.
...
- **Web UI.** CLI only.
- **High availability.** Single manager, sqlite, no replication. If the manager dies, existing peers continue working (their certs are still valid); only new enrollments and changes are blocked until it's restored.

## Build order

1. **`init` and `enroll`** — get a peer online end-to-end. CA generation, token issuance, HTTP endpoint, tarball assembly, peer-side install script. Self-enrollment of the manager.
2. **`list`** — read the db, print state. Sanity check that the data model works.
3. **Push infrastructure** — the SSH client code in the manager that connects to a peer using the manager's cert, writes files atomically, reloads sshd. This is the foundation for everything below.
...
- **Language**: Go. Single static binary, good ssh library (`golang.org/x/crypto/ssh`), easy to cross-compile if you ever want the manager on different architectures.
